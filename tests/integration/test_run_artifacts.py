...
from runtime.hooks.lifecycle import stop_report_hook
from runtime.hooks.policy import _cancel_task_for_terminal_deny
from runtime.operation import Operation
from runtime.security import PermissionBehavior, PermissionDecision
...
def test_stop_report_hook_continues_after_artifact_failure(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "runtime.hooks.lifecycle.ReadableTraceWriter", BrokenReadableTraceWriter
    )
...
def test_stop_report_writes_standard_run_artifacts(tmp_path) -> None:
    context = create_agent_session(
        repo_path=tmp_path,
        task="inspect the project",
        permission_mode="accept_edits",
        config=RunConfig(permission_mode="accept_edits"),
        initial_messages=[{"role": "user", "content": "inspect the project"}],
        system_prompt="system prompt",
        include_initial_message=True,
    )
    context.finished = True
    context.success = True
    context.final_text = "Inspection complete."

    stop_report_hook(context)

    assert context.run_dir.parent == tmp_path / ".agent" / "runs"
